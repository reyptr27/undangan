...
export const util = (() => {
  ...
  const guest = () => {
    ...
    div.innerHTML = `<p class="mt-0 mb-1 mx-0 p-0 text-light">${guest.getAttribute("data-message")}</p><h2 class="py-4 text-light border rounded">${escapeHtml(name)}</h2>`;
  ...
  // Function to set active link based on current URL or section in view
  const setActiveLink = () => {
          ...
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              navLinks.forEach((nav) => {
                ...
                nav.querySelector("i").style.color = ""; // Reset other icons
              });
              link.classList.add("active");
              link.querySelector("i").style.color = "rgb(198, 23, 23)"; // Set active icon color
            }
          });
        },
        { threshold: 0.5 }
